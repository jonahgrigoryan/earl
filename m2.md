...
- **Handle lifecycle**: Create once per symbol; release safely on EA deinit
- **Insufficient data**: If bars are insufficient, return safe defaults (e.g., NaN-guarded zeros) and log a warning
...
```cpp
// D1 timeframe (yesterday's D1 bar): |C_D1[1]-O_D1[1]| / max(H_D1[1]-L_D1[1], point)
double BTR = MathAbs(CloseD1[1] - OpenD1[1]) / MathMax(HighD1[1] - LowD1[1], _Point);
```
...
- **Overextension check**: Use spec-aligned defaults: RSI guard in ranges [35, 70] (default) or [30, 68] variant
- **Configurability**: Keep thresholds ready for optimization ranges in tester
...
- **Logging**: Record BTR, SDR, ORE, RSI, Bias, setupType, confidence, and gating reasons via decision logs
...
- **Next trade validation (M2 scope)**: `open_risk + pending_risk + next_trade_worst_case ≤ 0.9 * min(room_today, room_overall)`
- **Deferred to M7**: Second-trade session rule (0.8 multiplier) and explicit floor buffer checks will be implemented with the ensemble enhancements
...
M2 keeps the existing stubs only. Full meta-policy routing (BWISC vs MR tie-breakers, efficiency comparison) is deferred to M7 per finalspec.
